usage:
	...
	@echo "	make firstpatch - add the lewa hook into target framework smali code first time for a device"
	@echo "	make patchlewa  - incrementaly add the lewa hook into target framework smali code"
...
$(TMP_DIR)/apktool-if: $(ZIP_FILE) $(APKTOOL_IF_RESULT_FILE)/8.apk | $(TMP_DIR)
	@echo ">>> Install framework resources for apktool..."
	$(hide) for res_file in `find $(PORT_BUILD)/res/ -name "*.apk"`;do\
		$(APKTOOL) if $$res_file; \
	done
	@echo install lewa-res.apk
	$(APKTOOL) if $(SYSOUT_DIR)/framework/lewa-res.apk
	$(APKTOOL) if $(SYSOUT_DIR)/framework/framework-res.apk lewa
...
firstpatch:
	$(PATCH_LEWA_FRAMEWORK) $(PORT_ROOT)/android/google-framework $(PORT_ROOT)/android `pwd`

# Target to incrementaly add lewa hook into target framework
patchlewa:
	$(PATCH_LEWA_FRAMEWORK) $(PORT_ROOT)/android/last-framework $(PORT_ROOT)/android `pwd`
	@echo Patchlewa completed!

# Target to release LEWA jar and apks
release: $(RELEASE_LEWA) release-framework-base-src
...
release-framework-base-src: release-lewa-resources
...
verify: $(ERR_REPORT)
	...
	@echo "local-lewa-apps       = $(local-lewa-apps)"
	...
	@echo "RELEASE_LEWA    = $(RELEASE_LEWA)"
	@echo "LEWAAPPS_MOD    = $(LEWAAPPS_MOD)"
	@echo "lewa-apps       = $(private-lewa-apps)"
	@echo "LEWAAPPS        = $(LEWAAPPS)"
...
ota-base-restore: $(use_previous_target_file)
	...
	for app in $(LEWAAPPS_MOD) $(APPS); do \
